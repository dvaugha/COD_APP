const App = {
            ...
            init: function () {
                ...
                try {
                    let s = localStorage.getItem('COD_GOLF_DATA_v281_0');
                    if (!s) {
                        const old = localStorage.getItem('COD_GOLF_DATA_v277_0') ||
                                    localStorage.getItem('COD_GOLF_DATA_v276_0') ||
                    ...
                    if (!this.d.nassauPresses) this.d.nassauPresses = [];
                    ...
                    if (this.d.noHandicaps === undefined) this.d.noHandicaps = false;
                } catch (e) { }
            ...
            save: (function () {
                ...
                return function () {
                    clearTimeout(t);
                    t = setTimeout(() => {
                        localStorage.setItem('COD_GOLF_DATA_v281_0', JSON.stringify(this.d));
                    }, 500);
                };
            })(),
            ...
            hardReset: function () {
                if (confirm("This will Delete History and Reset Roster. Continue?")) {
                    localStorage.removeItem('COD_GOLF_DATA_v281_0');
            ...
            toggleNoHcps: function (val) {
                this.d.noHandicaps = val;
                this.save();
            },
            ...
            navH: function (d) {
                ...
                const banner = document.getElementById('auto-adv-banner');
                ...
                if (isNaN(curH) || isNaN(startH)) {
                    console.error("Navigation Error: curH or startH is NaN", {h: this.d.h, start: this.d.start});
                    return;
                }
                ...
                if (d === 1 && (!this.d.s[n] || Object.keys(this.d.s[n]).length === 0)) {
                    this.corr = false;
                }
                ...
                if (d === 1 && !this.corr) {
                    this.strokesTmr = setTimeout(() => {
                        ...
                        if (strokedPlayers.length > 0) {
                            let namesStr = "";
                            if (strokedPlayers.length === 1) namesStr = strokedPlayers[0] + " has";
                            else if (strokedPlayers.length === 2) namesStr = strokedPlayers[0] + " and " + strokedPlayers[1] + " have";
                            else if (strokedPlayers.length === 3) namesStr = strokedPlayers[0] + ", " + strokedPlayers[1] + " and " + strokedPlayers[2] + " have";
                            else if (strokedPlayers.length === 4) namesStr = "Everyone has";
                            
                            this.speak(namesStr + " strokes on this next hole. Be sure everyone knows!");
                        }
                    }, 15000);
                }
            ...
            announceNextTee: function() {
                ...
                if (isGold) {
                    if (this.d.crs === 're') {
                        this.speak("Attention! Next hole is a Green tee. Move to the green markers.");
                    } else {
             ...
             mod: function (idx, delta) {
                ...
                if (this.d.gameType === 'cc_match') {
                    const p1 = this.slotMap[0];
                    const p2 = this.slotMap[1];
                    const p3 = this.slotMap[2];
                    const p4 = this.slotMap[3];
                    if (idx === 0) {
                        this.d.s[this.d.h][p1] = s;
                        this.d.s[this.d.h][p2] = s;
                    } else if (idx === 2) {
                        this.d.s[this.d.h][p3] = s;
                        this.d.s[this.d.h][p4] = s;
                    }
                }
            ...
            checkAuto: function () {
                ...
                const indices = this.d.gameType === 'cc_match' ? [0, 2] : [0, 1, 2, 3];
                indices.forEach(i => {
                    if (this.d.ps[i]) {
                        activeCount++;
                ...
                if (activeCount > 0 && enteredCount === activeCount && allPositive) {
                    const banner = document.getElementById('auto-adv-banner');
            ...
            startRound: function (keepScores) {
                try {
                    ...
                    const gt = gtLine ? gtLine.value : (this.d.gameType || 'cod');
                    ...
                    } else if (gt === 'nassau' || gt === 'cc_match') {
                        if (count < 4) { alert("Nassau and Captain's Choice require 4 players (Cart A vs Cart B)!"); return; }
                    } else {
                    ...
                    const sCourseEl = document.getElementById('s-course');
                    if (searchEl && searchEl.value && sCourseEl && sCourseEl.value && CS[sCourseEl.value]) {
                        this.d.crs = sCourseEl.value;
                    } else if (!this.d.crs || !CS[this.d.crs]) {
                        this.d.crs = 'cc';
                    }

                    const teeEl = document.getElementById('s-tee');
                    if (teeEl && teeEl.value) this.d.tee = teeEl.value;
                    const betEl = document.getElementById('s-bet');
                    if (betEl && betEl.value !== '') this.d.bet = parseInt(betEl.value);
                    const startEl = document.getElementById('s-start');
                    if (startEl && startEl.value !== '') this.d.start = parseInt(startEl.value);
                    this.d.gameType = gt;
                    const potEl = document.getElementById('s-pot');
                    if (potEl && potEl.value !== '') this.d.pot = parseInt(potEl.value) || 20;
                    const hcpModeEl = document.getElementById('s-hcp-mode');
                    if (hcpModeEl && hcpModeEl.value) this.d.hcpMode = hcpModeEl.value;
                    const noHcpsEl = document.getElementById('s-no-hcps');
                    if (noHcpsEl) this.d.noHandicaps = noHcpsEl.checked;
                    ...
                    if (!keepScores) {
                        ...
                        this.d.nassauPresses = [];
                        this.d.rabbitHistory = {};
                        this.d.h = parseInt(this.d.start) || 1;
                    }

                    // Composite Course Logic
                    const crsData = CS[this.d.crs] || CS['cc'];
                    if (crsData && crsData.nines) {
                        const n1El = document.getElementById('s-n1');
                        const n2El = document.getElementById('s-n2');
                        const n1 = n1El ? n1El.value : 'n1';
                        const n2 = n2El ? n2El.value : 'n2';
                        this.d.nines = [n1, n2];
                ...
                if (c && c.y && (!c.y.combo || c.y.combo.length === 0)) {
                    ...
                    if (hasCmb || hasHcp) {
                        ...
                        for (let i = 0; i < 18; i++) {
                            ...
                            if (id === 're' && c.y.green && c.y.white) {
                                c.y.combo[i] = useGold ? (c.y.green[i] || 0) : (c.y.white[i] || 0);
                            } else if (c.y.gold && c.y.white) {
            ...
            getPops: function (pIdx, hIdx, useFull) {
                if (this.d.noHandicaps) return 0;
                ...
                const course = CS[this.d.crs] || CS['cc'];
            ...
            initiateNassauPress: function (team) {
                const rh = this.getRelHole(this.d.h);
                const isFront = (rh <= 9);
                const curNine = isFront ? 'front' : 'back';
                const nineLabel = isFront ? 'Front 9' : 'Back 9';

                if (!this.d.nassauPresses) this.d.nassauPresses = [];
                if (this.d.nassauPresses.some(p => p.nine === curNine && p.team === team)) {
                    alert(`A ${nineLabel} press is already active for this team!`);
                    return;
                }

                const nr = this.calcNassau();
                const activeSeg = isFront ? nr.front : nr.back;
                const diff = (team === 1) ? (activeSeg.w1 - activeSeg.w2) : (activeSeg.w2 - activeSeg.w1);
                if (diff >= 0) {
                    alert("Team must be trailing in this nine to press!");
                    return;
                }
                ...
                const tNames = (team === 1 ? t1 : t2).map(idx => this.d.ps[idx]);
                const nameStr = tNames.join(" & ");
                const startAbs = this.getAbsHole(rh);
                const endAbs = isFront ? this.getAbsHole(9) : this.getAbsHole(18);

                const startThePress = () => {
                    this.d.nassauPresses.push({
                        nine: curNine,
                        team: team,
                        startH: rh,
                        bet: 5
                ...
                let warningHtml = `
                    <div style="color:white; font-weight:700; font-size:16px; margin-bottom:8px;">Initiate $5 ${nineLabel} Press?</div>
                    <div style="color:#94A3B8; font-size:13px; margin-bottom:12px;">${nameStr} trailing by ${Math.abs(diff)} in the ${nineLabel}.</div>
                    <div style="color:#10B981; font-weight:700; background:rgba(16,185,129,0.1); border:1px solid #10B981; padding:8px; border-radius:6px; font-size:12px;">
                        Scores Holes ${startAbs} through ${endAbs} for a new $5 bet.
                    </div>
                `;
                let voiceMsg = `Press initiated on hole ${startAbs} for the ${nineLabel}.`;
            ...
            uDash: function () {
                ...
                if (this.d.tee === 'combo' && c) { 
                    ...
                    if (this.d.crs === 're') {
                        tT = useGold ? "GREEN TEE" : "WHITE TEE"; 
                    } else {
                ...
                if (teeInd) {
                    teeInd.innerText = tT;
                    teeInd.style.color = (tT.includes('GOLD') ? '#FFD700' : tT.includes('GREEN') ? '#4ade80' : '#FFFFFF');
                }
                ...
                // RELATIVE HOLE LOGIC
                let m = Math.floor((rh - 1) / 6);
                if (this.d.gameType === 'nassau' || this.d.gameType === 'cc_match') m = 0;
                ...
                else if (this.d.gameType === 'cc_match') fmt = "CAPTAIN'S CHOICE";
                else if (this.d.gameType === 'rabbit') fmt = "RABBIT HUNTER";
                ...
                const getStatStr = (res, label) => {
                    ...
                    if (this.d.gameType === 'scramble') {
                        ...
                        let d = res.w1 - res.w2;
                        if (d > 0) s = `${sn(rt1[0])}/${sn(rt1[1])} <em style="color:#60A5FA">${d} UP</em>`;
                ...
                if (this.d.gameType === 'stroke' || this.d.gameType === 'single') {
                    ...
                    const nassauLine = (label, seg) => {
                        ...
                        let val = 'ALL SQUARE';
                        if (d > 0) val = `${tn1} <em style="color:#60A5FA">${d} UP</em>`;
                    ...
                    if (nr.presses && nr.presses.length > 0) {
                        nr.presses.forEach(p => {
                            statHTML += nassauLine(p.name, p);
                        });
                    }
                } else if (this.d.gameType === 'rabbit') {
                    statHTML = ""; // Rabbit banner is displayed independently above.
                } else if (main) {
                ...
                // PRESS BUTTONS
                let pressButtons = '';
                if (this.d.gameType === 'cc_match' || this.d.gameType === 'nassau') {
                    const isFront = (rh <= 9);
                    const curNine = isFront ? 'front' : 'back';
                    const nr = this.calcNassau();
                    const activeSeg = isFront ? nr.front : nr.back;
                    const diff1 = activeSeg.w1 - activeSeg.w2;
                    const diff2 = activeSeg.w2 - activeSeg.w1;
                    const hasP1 = this.d.nassauPresses && this.d.nassauPresses.some(p => p.nine === curNine && p.team === 1);
                    const hasP2 = this.d.nassauPresses && this.d.nassauPresses.some(p => p.nine === curNine && p.team === 2);
                    if (diff1 < 0 && !hasP1) pressButtons += `<button class="g-btn g-btn-sm" style="background:#EF4444; width:auto; margin-top:4px; margin-right:4px; padding:6px 10px; font-size:10px;" onclick="App.initiateNassauPress(1)">${sn(nr.t1[0])}/${sn(nr.t1[1])} PRESS</button>`;
                    if (diff2 < 0 && !hasP2) pressButtons += `<button class="g-btn g-btn-sm" style="background:#F59E0B; width:auto; margin-top:4px; padding:6px 10px; font-size:10px;" onclick="App.initiateNassauPress(2)">${sn(nr.t2[0])}/${sn(nr.t2[1])} PRESS</button>`;
                } else if (main && this.d.gameType !== 'scramble' && this.d.gameType !== 'stroke' && this.d.gameType !== 'rabbit') {
                ...
                const isCC = (this.d.gameType === 'cc_match');
                const map = (id, pid) => {
                    this.slotMap[id] = pid;
                    ...
                    if (isCC) {
                        if (id === 0) displayName = `${this.d.ps[0]} & ${this.d.ps[1]}`;
                        else if (id === 2) displayName = `${this.d.ps[2]} & ${this.d.ps[3]}`;
                    }
                    ...
                    if (this.d.gameType === 'single') {
                        ...
                        nmEl.innerHTML = `${displayName} <span style="color:#10B981; font-weight:900; margin-left:8px; font-size:16px;">${sStr}</span>`;
                    } else {
                        ...
                        const junkStr = isCC ? "" : `<span style="font-size:9px; color:#FFFFFF; margin-left:8px;">🟢${jk.G} 🏖️${jk.S} LP${jk.P}</span>`;
                        let rabbitIcon = '';
                        ...
                        nmEl.innerHTML = `${displayName}${rabbitIcon}${dots} ${junkStr} <span style="color:${expCol}; background:${expBg}; border:1px solid ${expCol}44; font-weight:900; font-size:10px; padding:2px 6px; border-radius:6px; margin-left:4px; vertical-align:middle;">$${expTxt}</span>`;
                    }
                    ...
                    const sc = (this.d.s[this.d.h] && this.d.s[this.d.h][pid]) !== undefined ? this.d.s[this.d.h][pid] : (isCC ? par : undefined);
            ...
            calcJunkRes: function () {
                ...
                segs.forEach((sig, segIdx) => {
                    ...
                    [1, 2, 3, 4, 5, 6].forEach(rh => {
                        const relH = (segIdx * 6) + rh;
                        const absH = this.getAbsHole(relH);
                        const c = CS[this.d.crs] || CS['cc'];
                        if (!c || !c.p) return;
                        const par = c.p[absH - 1];
                        if (this.d.junk && this.d.junk[absH]) {
                            players.forEach(pIdx => {
                                ['G', 'S', 'P'].forEach(t => {
                                    if (this.d.junk[absH][pIdx + '_' + t]) {
                                        const sc = (this.d.s[absH] && this.d.s[absH][pIdx]);
                                        // RULE: G and S require Par or BETTER
            ...
            getScorecardHTML: function () {
                ...
                const build9 = (startH, label) => {
                    ...
                    let pRows = '';
                    const isCC = (this.d.gameType === 'cc_match');
                    const playerIndices = isCC ? [0, 2] : [0, 1, 2, 3];
                    playerIndices.forEach(pIdx => {
                        if (!this.d.ps[pIdx]) return;
                        let nameLabel = this.d.ps[pIdx].substring(0, 8);
                        if (isCC) {
                            if (pIdx === 0) nameLabel = `${this.d.ps[0].substring(0, 3)}/${this.d.ps[1].substring(0, 3)}`;
                            else if (pIdx === 2) nameLabel = `${this.d.ps[2].substring(0, 3)}/${this.d.ps[3].substring(0, 3)}`;
                        }
                        let r = `<tr><td>${nameLabel}</td>`;
                        let tot = 0;
                        for (let n = startH; n < startH + 9; n++) {
            ...
            getGrossTotalsHTML: function () {
                ...
                playerIndices.forEach(i => {
                    ...
                    let nameLabel = this.d.ps[i].substring(0, 8);
                    if (isCC) {
                        if (i === 0) nameLabel = `${this.d.ps[0]} & ${this.d.ps[1]}`;
                        else if (i === 2) nameLabel = `${this.d.ps[2]} & ${this.d.ps[3]}`;
                    }
                    totHTML += `<div style="display:flex; justify-content:space-between; align-items:center; background:#0F172A; padding:8px 12px; border-radius:8px; border:1px solid #334155;"><span style="font-size:12px; font-weight:700; color:#FFFFFF; text-transform:uppercase;">${nameLabel}</span><span style="font-size:15px; font-weight:900; color:white;">${t || '-'}</span></div>`;
                });
                totHTML += '</div></div>';
                return totHTML;
            },
            ...
            uCard: function () {
                ...
                if (this.d.gameType === 'stroke') {
                    ...
                    const segLine = (label, seg) => {
                        if (seg.winner === 1) return `<div style="display:flex;justify-content:space-between;font-size:13px;font-weight:700;margin-bottom:6px;"><span style="color:#FFFFFF;">${label}:</span><span style="color:#60A5FA;">${tn1} wins +$${bet}</span></div>`;
                    ...
                    const title = this.d.gameType === 'cc_match' ? "CAPTAIN'S CHOICE RESULTS" : "NASSAU RESULTS";
                    const titleCol = this.d.gameType === 'cc_match' ? "#F59E0B" : "#60A5FA";
                    resHTML += `<div style="margin-bottom:16px;background:rgba(96,165,250,0.1);padding:10px;border-radius:8px;">`
                        + `<div style="font-weight:900;color:${titleCol};font-size:14px;text-align:center;margin-bottom:10px;">🏆 ${title}</div>`
                    ...
                    ["CARTS (1-6)", "OPPOSITES (7-12)", "DRIVERS (13-18)"].forEach((lbl, idx) => {
                        ...
                        results.forEach((r, rIdx) => {
                            ...
                            if (r.winner === 1) { wTxt = `${this.d.ps[r.seg.t1[0]]}/${this.d.ps[r.seg.t1[1]]} (+$${r.amt})`; wCol = "#60A5FA"; }
                            ...
                            for (let hRel = start; hRel <= end; hRel++) {
                                ...
                                if (best1 < best2) { bStyle = "background:#60A5FA; color:#0F172A;"; bTxt = `H${h}: ${best1}`; }
            ...
            share: async function () {
                ...
                try {
                    ...
                    sn.innerHTML = `
                        ...
                        <div class="sn-sub">${new Date().toLocaleDateString()} • ${this.d.tee.toUpperCase()} • COD GOLF v281.1</div>
                        <div class="sn-sect">
            ...
            calcBets: function () {
                ...
                if (this.d.gameType === 'stroke') {
                    ...
                    const segs = [
                        { name: "FRONT 9", seg: nr.front, bet: bet },
                        { name: "BACK 9", seg: nr.back, bet: bet },
                        { name: "TOTAL", seg: nr.overall, bet: bet }
                    ];

                    if (nr.presses && nr.presses.length > 0) {
                        nr.presses.forEach(p => {
                            segs.push({ name: p.name, seg: p, bet: p.bet || 5 });
                        });
                    }
                    ...
                    segs.forEach(s => {
                        ...
                        const sBet = s.bet;
                        if (s.seg.winner === 1) {
                            nr.t1.forEach(p => { bets[p] += sBet; logs[p].push(`${s.name}: +$${sBet}`); });
                            nr.t2.forEach(p => { bets[p] -= sBet; logs[p].push(`${s.name}: ($-${sBet})`); });
                            wStr = `${tn1} (+$${sBet})`;
                        } else if (s.seg.winner === 2) {
                            nr.t2.forEach(p => { bets[p] += sBet; logs[p].push(`${s.name}: +$${sBet}`); });
                            nr.t1.forEach(p => { bets[p] -= sBet; logs[p].push(`${s.name}: ($-${sBet})`); });
            ...
            showStandings: function () {
                ...
                if (gt === 'cod' || gt === 'scramble') {
                    ...
                    for (let si = 0; si <= curSeg; si++) {
                        ...
                        if (d > 0)       { statusTxt = `${n1} leads ${d} UP`; statusCol = '#60A5FA'; }
                ...
                if (gt === 'nassau') {
                    ...
                    const nassauStdRow = (label, seg) => {
                        const d = seg.w1 - seg.w2;
                        let txt, col;
                        if (d > 0)      { txt = `${tn1} leads ${d} UP`; col = '#60A5FA'; }
            ...
            calcNassau: function () {
                ...
                for (let h = 1; h <= 18; h++) {
                    ...
                    if (!s) continue;
                    if (this.d.gameType === 'cc_match') {
                        if (s[0] === undefined || s[2] === undefined) continue;
                    } else {
                        if (Object.keys(s).length < activeCount) continue;
                    }
                ...
                const presses = [];
                if (this.d.nassauPresses && this.d.nassauPresses.length > 0) {
                    this.d.nassauPresses.forEach(p => {
                        let pW1 = 0, pW2 = 0;
                        const startH = p.startH;
                        const endH = (p.nine === 'front' ? 9 : 18);
                        for (let rh = startH; rh <= endH; rh++) {
                            const absH = this.getAbsHole(rh);
                            const s = this.d.s[absH];
                            if (!s) continue;
                            if (this.d.gameType === 'cc_match') {
                                if (s[0] === undefined || s[2] === undefined) continue;
                            } else {
                            ...
                            const best1 = Math.min(
                                (s[t1[0]] || 99) - this.getPops(t1[0], absH - 1),
                                (s[t1[1]] || 99) - this.getPops(t1[1], absH - 1)
                            );
                            const best2 = Math.min(
                                (s[t2[0]] || 99) - this.getPops(t2[0], absH - 1),
                                (s[t2[1]] || 99) - this.getPops(t2[1], absH - 1)
                            );
                            if (best1 < best2) pW1++;
                            else if (best2 < best1) pW2++;
                        }
                        const winner = pW1 > pW2 ? 1 : (pW2 > pW1 ? 2 : 0);
                        const startAbs = this.getAbsHole(startH);
                        const endAbs = this.getAbsHole(endH);
                        const name = `${p.nine === 'front' ? 'FRONT 9' : 'BACK 9'} PRESS (H${startAbs}-${endAbs})`;
                        presses.push({
                            ...p,
                            w1: pW1,
                            w2: pW2,
            ...
            shareSimpleScorecard: async function () {
                const btn = document.getElementById('sms-score-btn');
                try {
                    ...
                    node.innerHTML = `
                        ...
                        <div style="margin-top:20px; text-align:center; font-size:10px; color:#475569;">GENERATED BY COD GOLF v281.1</div>
                    `;

                    // Inject necessary table styles for the snapshot
                    const style = document.createElement('style');
                    style.innerHTML = `
                        #share-node table { width: 100%; border-collapse: collapse; margin-bottom: 0; }
                        #share-node th, #share-node td { border: 1px solid #334155; text-align: center; padding: 6px 2px; color: white; font-size: 11px; }
                        #share-node th { background: #1E293B; color: #10B981; font-weight: 900; }
                        #share-node .par-row { background: #0F172A; font-weight: 700; color: #94A3B8; }
                        #share-node .sc-val { font-weight: 900; font-size: 14px; position:relative; z-index:2; }
                        #share-node .sc-bird { color: #10B981; }
                        #share-node .sc-bog { color: #F87171; }
                        #share-node .sc-dbl { color: #EF4444; }
                        #share-node .sc-eagle { color: #22D3EE; text-decoration: underline; }
                        #share-node .card-table-wrap { overflow: hidden; border-radius: 8px; border: 1px solid #334155; }
                    `;
